...
(az aks create -n testaks001 -g $1 --network-plugin azure \
    --enable-managed-identity -a ingress-appgw \
    --appgw-name myApplicationGateway --appgw-subnet-cidr "10.2.0.0/16" \
    --generate-ssh-keys \
...
echo "${command}: - enable K8s system..."
(az aks get-credentials -n testaks001 -g $1 --overwrite-existing) > ${tmpFile} 2>&1
if [ $? -gt 0 ]; then
    cat "${tmpFile}"
    rmFile "${tmpFile}"
    return 1
